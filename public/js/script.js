...
const submitForm = async body => {
	const res = await fetch('http://127.0.0.1:9005/addNewVisitor', {
		method: 'post',
...
const deleteTableRow = async id => {
	...
	// Send request
	const res = await fetch(`http://127.0.0.1:9005/deleteVisitor/${id}`, {
		method: 'delete'
	});
...
const init = async () => {
	...
	const res = await fetch('http://127.0.0.1:9005/listVisitors');
	const data = await res.json();
